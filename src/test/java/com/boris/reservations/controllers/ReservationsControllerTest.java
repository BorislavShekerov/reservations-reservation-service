...
public class ReservationsControllerTest extends ReservationBaseTest{
	...
	public void getReservationsForUser() throws Exception {
		...
		List<Reservation> result = this.testObj.getUserReservations();
	...
	public void getFreeTablesForVenue() throws Exception {
		when(reservationService.getFreeTablesForVenue(DUMMY_VENUE_ID, DUMMY_RESERVATION_DATE, DUMMY_RESERVATION_PEOPLE_ATTENDING)).thenReturn(Optional.ofNullable(null));

		List<Table> freeTables = this.testObj.getFreeTablesForVenue(DUMMY_VENUE_ID, DUMMY_RESERVATION_DATE.toString(), DUMMY_RESERVATION_PEOPLE_ATTENDING);
		
		verify(reservationService).getFreeTablesForVenue(DUMMY_VENUE_ID, DUMMY_RESERVATION_DATE, DUMMY_RESERVATION_PEOPLE_ATTENDING);
	...
	public void getAllReservationsForVenue() throws Exception {
		when(reservationService.getAllReservationsForVenue(DUMMY_VENUE_ID, DUMMY_RESERVATION_DATE)).thenReturn(Optional.of(Arrays.asList(DUMMY_RESERVATION)));
		
		List<Reservation> result = this.testObj.getAllReservationsForVenue(DUMMY_VENUE_ID, DUMMY_RESERVATION_DATE.toString());
	...
	public void addReservation_successful() throws Exception {
		when(reservationService.saveReservation(DUMMY_RESERVATION)).thenReturn(Optional.of(DUMMY_RESERVATION));
		
		Reservation reservationAdded = this.testObj.addReservation(DUMMY_RESERVATION);
	...
	public void addReservation_unsuccessful() throws Exception {
		when(reservationService.saveReservation(DUMMY_RESERVATION)).thenReturn(Optional.ofNullable(null));
		
		Reservation result = this.testObj.addReservation(DUMMY_RESERVATION);
